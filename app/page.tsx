...
export default function Home() {
  ...
  // NUEVO: Para cambiar entre Home y Catálogo Completo
  ...
  // Mostramos solo 3 productos como destacados en la Home
  const productosPromocion = PRODUCTS.slice(0, 3);
  ...
  return (
    <main className="min-h-screen bg-[#fdfcf0] pb-20 font-sans text-gray-800">
      ...
      <nav className="bg-[#4a5d23] text-white p-4 shadow-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-4">
          ...
          <div className="hidden md:flex gap-6 text-xs uppercase tracking-widest font-medium">
            <button onClick={() => setVerTienda(false)} className="hover:text-amber-200 uppercase">Inicio</button>
                  ...
                  {categorias.map((cat) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setVerTienda(true); // Al elegir categoría, vamos a la tienda
      ...
      {/* Carrito flotante (Igual que antes) */}
      {mostrarResumen && (
      ...
      {!verTienda ? (
        ...
        <>
          ...
          <section id="promos" className="max-w-6xl mx-auto p-4 pt-16 text-center">
            <h3 className="text-3xl font-bold mb-10">🔥 Promociones Destacadas</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {productosPromocion.map((producto) => (
                <div key={producto.id} className="bg-white rounded-3xl overflow-hidden shadow-lg border border-gray-100 p-4">
            ...
            <button 
              ...
              className="mt-12 bg-white border-2 border-[#4a5d23] text-[#4a5d23] px-8 py-3 rounded-full font-bold hover:bg-[#4a5d23] hover:text-white transition-all shadow-md"
            >
        ...
        <section id="productos" className="max-w-6xl mx-auto p-4 pt-16 min-h-screen">
          <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
            <button onClick={() => setVerTienda(false)} className="text-[#4a5d23] font-bold underline">← Volver al inicio</button>
            <div className="relative w-full md:w-64">
              <span className="absolute inset-y-0 left-3 flex items-center text-gray-400">🔍</span>
              <input 
                type="text" 
                placeholder="Buscar en la tienda..." 
                className="w-full pl-10 pr-4 py-2 rounded-full border-2 border-gray-200 outline-none focus:border-[#4a5d23]"
                onChange={(e) => setBusqueda(e.target.value)}
              />
            </div>
          </div>
          
          <h3 className="text-3xl font-bold mb-8">Catálogo: {categoriaSeleccionada}</h3>
      ...
      <a href="https://wa.me/5493515416836" target="_blank" className="fixed bottom-6 left-6 bg-[#25d366] text-white p-4 rounded-full shadow-2xl z-[100] hover:scale-110 transition-transform">
